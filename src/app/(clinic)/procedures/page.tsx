...
function getConditionLabel(condition: ResultingCondition | string | undefined, t: ReturnType<typeof useTranslations>) {
  switch (condition) {
    case ToothCondition.FILLING: return t("condition.filling");
    case ToothCondition.CROWN: return t("condition.crown");
    case ToothCondition.IMPLANT: return t("condition.implant");
    case ToothCondition.ROOT_CANAL: return t("condition.rootCanal");
    case ToothCondition.EXTRACTED: return t("condition.extracted");
    case ToothCondition.MISSING: return t("condition.missing");
...
export default function ProceduresPage() {
  ...
  async function handleDelete(procedure: DentalProcedure) {
     ...
     */
    const ok = window.confirm(t("toast.deleteConfirm", { name: procedure.name }));
    if (!ok) return;
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between">
              <p className="text-sm font-bold text-slate-500">{t("stats.total")}</p>
          ...
          <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between">
              <p className="text-sm font-bold text-slate-500">{t("stats.totalPrice")}</p>
          ...
          <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-3 text-3xl font-black text-emerald-600">{t("stats.active")}</p>
          </div>
        </div>
        ...
        <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="text-xl font-black text-slate-950">{t("list.title")}</h2>
            ...
            <table className="w-full border-collapse text-left">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="px-5 py-4 text-right text-xs font-black uppercase tracking-wide text-slate-500">{t("table.actions")}</th>
                </tr>
              </thead>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-2xl overflow-hidden rounded-[32px] border border-white/40 bg-white shadow-2xl shadow-slate-950/20">
              ...
              <div className="relative flex items-start justify-between gap-4">
                <div>
                  <div className="inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-1.5 text-xs font-black text-white ring-1 ring-white/20">
                    <Sparkles size={15} />
                    {editingProcedure ? t("modal.editModeBadge") : t("modal.createModeBadge")}
                  </div>
            ...
            <div className="space-y-5 p-6">
              ...
              <div>
                <label className="mb-2 block text-sm font-black text-slate-700">{t("modal.resultingConditionLabel")}</label>
              ...
              <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                ...
                <div className="mt-3 flex flex-col gap-3 rounded-2xl bg-white p-4 md:flex-row md:items-center md:justify-between">
                  <div>
                    <p className="text-sm font-black text-slate-950">{form.name || t("modal.previewNameFallback")}</p>
                    <p className="mt-1 text-xs font-bold text-slate-400">{form.code || t("modal.previewCodeFallback")}</p>
                  </div>
